...
document.getElementById('bmiform').addEventListener('submit', function(e) {
    ...
    // Mengambil nilai berat dan tinggi badan dari input, lalu dikonversi menjadi float
    const weight = parseFloat(document.getElementById('weight').value);
    const height = parseFloat(document.getElementById('height').value) / 100; // Mengonversi tinggi dari cm ke meter

    // Validasi input untuk memastikan valid
    if (isNaN(weight) || isNaN(height) || weight <= 0 || height <= 0) {
        alert("Masukkan nilai yang valid untuk berat dan tinggi badan."); // Menampilkan alert jika  tidak valid
    ...
    // Menentukan kategori dan saran dari tabel BMI
    if (bmi < 18.5) {
    ...
    bmiSummary.innerHTML = `
        ...
        ${second ? `<p>${second}</p>` : ''}
    `;
    showBMISuggestions(bmi); // Memanggil fungsi untuk menampilkan saran  berdasarkan BMI
});
